...
namespace diskann {
  ...
  void DeleteSet::batch_is_dead(const uint32_t *ids, bool* dead, const uint32_t count) {
    std::lock_guard<std::mutex> l(this->lock);
    if(this->secondary->empty()) {
      for(uint32_t i=0;i<count;i++) {
        ...
        for(uint32_t i=0;i<count;i++) {
          // fast short-circuit if empty
          dead[i] = (this->primary->find(ids[i]) != this->primary->end()) || (this->secondary->find(ids[i]) != this->secondary->end()); 
        }
        return ;
      }
    }
